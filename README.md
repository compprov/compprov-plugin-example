...
- [`Currency`](src/main/java/io/compprov/examples/nav/model/Currency.java) — `BTC`, `WBTC`, `ETH`,
  `USDC`, `USDT`, `USD`, `WSTETH`, each with its own decimal precision. `WBTC` is tracked
...
One file per domain (`payroll.json`, `insurance.json`, `idealgas.json`, ...), named after the
domain's test package — every calculation here is deliberately correct, with no injected defects,
to serve as true-negative fixtures. The three NAV scenarios keep their original names
(`profit.json`, `payout.json`, `net_asset_value.json`). `metrology.json` is the one exception: it
comes from a `compprov-core` test case rather than this project, so there's no local generator for
it.
...
`malicious/legacy/` holds fixtures that predate the per-domain generators and have no
corresponding `@Test` class in this repo — `broken-chronology.json`,
`ciddor-substitution-1-1.json`, `ciddor-substitution-similar.json` (constructed by hand), and
...
alarm — not because the underlying calculation was wrong, but because the model surfaced a real,
...
- **`net_asset_value.json`** — Precision tampering: `SUSPICIOUS LOGIC (74.0)`. Every
  ...
  round-down-always policy, not a bug. But that policy used to live only inside `Amount`, invisible
  to the CPG, so the model had no in-graph evidence the truncation was intentional and flagged it
  for review. **Fixed**: every `WrappedAmount` operation descriptor
  ([`WrappedAmount`](src/main/java/io/compprov/examples/nav/wrapped/WrappedAmount.java)) now
  carries an explicit `rounding` metadata entry documenting the always-round-down policy, giving
  the model the declared justification it was missing.
- **`profit.json`** — Precision tampering: `SUSPICIOUS LOGIC (58.0)`, the same truncate-vs-round
  gap as `net_asset_value.json`, now fixed the same way. Semantic violation: `CONTEXT MISMATCH
  (63.0)` — [`ProfitCalculator`](src/test/java/io/compprov/examples/nav/ProfitCalculator.java)
  labeled one yield `"wBTC yield"` but stored it as a plain `BTC`-currency `Amount`, since
  ...
  assumption. **Fixed**: `Currency` now has a distinct `WBTC` entry, `ProfitDataProvider` reports
  the AAVE yield in `WBTC`, and `ProfitCalculator` (and its malicious siblings) convert it through
  an explicit `WBTC/BTC` peg `Rate` before the `BTC/USDC` conversion — the same pattern already
  used for `WSTETH`/`ETH` elsewhere in this example.

The model itself down-graded both verdicts from a confirmed-fraud category (`ANOMALY DETECTED` /
`SEMANTIC CAST DETECTED`) to a lower-confidence advisory one (`SUSPICIOUS LOGIC` / `CONTEXT
MISMATCH`) after checking materiality and scalability — the intended behavior for findings a human
reviewer should be able to dismiss quickly, rather than a silent false negative — but the source
fixes above should now keep `net_asset_value.json` and `profit.json` fully `CLEAN` on a re-run.
