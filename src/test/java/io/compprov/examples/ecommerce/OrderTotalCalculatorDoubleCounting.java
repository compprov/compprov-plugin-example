...
/**
 * Topological Accumulation Fraud (Double Counting) attack: the LOYALTY5 discount is applied once
 * inside the normal sequential discount chain (correctly), then subtracted a second time as a
 * standalone flat "loyalty discount" line at the end — deflating the reported order total by
 * that amount twice over.
 */
public class OrderTotalCalculatorDoubleCounting {
    ...
    public void calculate() {
        ...
        final var loyaltyDiscountAmount = afterDiscount1.multiply(discount2Rate, mc, descriptor("Loyalty discount"));
        final var orderTotal = orderTotalBeforeLoyaltyDiscount.subtract(loyaltyDiscountAmount, mc, descriptor("Order total"));
