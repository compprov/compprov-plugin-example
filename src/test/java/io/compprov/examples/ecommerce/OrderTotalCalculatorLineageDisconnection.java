...
/**
 * Lineage Disconnection / Context Substitution attack: {@code taxableAmount} and {@code tax} are
 * both computed correctly and replay cleanly in isolation — but the reported {@code orderTotal}
 * charged to the customer is a freshly-wrapped literal with no operation edge back to either of
 * them.
 */
public class OrderTotalCalculatorLineageDisconnection {
    ...
    public void calculate() {
        ...
        final var taxableAmount = discountedSubtotal.add(shipping, mc, descriptor("Taxable amount (computed, unused)"));

        final var taxRate = ctx.wrapBigDecimal(dp.fetchTaxRate(), descriptor("Tax rate (8%, California)"));
        final var tax = taxableAmount.multiply(taxRate, mc, descriptor("Sales tax (computed, unused)"));

        // === Reported order total: a disconnected literal standing in for taxableAmount + tax ===
        final var orderTotal = ctx.wrapBigDecimal(new BigDecimal("223.96"), descriptor("Order total"));
