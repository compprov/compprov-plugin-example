...
/**
 * Calculation Omission attack: a restocking fee — revenue collected on a partial return — is
 * fetched and correctly taxed in its own subgraph, but that subgraph is silently dropped from
 * the final {@code orderTotal} aggregation, understating what the customer was actually charged.
 */
public class OrderTotalCalculatorOmission {
    ...
    public void calculate() {
        ...
        // === Restocking fee revenue: computed and taxed correctly, but never folded in below ===
        final var restockingFee = ctx.wrapBigDecimal(dp.fetchRestockingFee(), descriptor("Restocking fee (revenue)"));
        final var restockingFeeTax = restockingFee.multiply(taxRate, mc, descriptor("Tax on restocking fee"));
        final var restockingFeeTotal = restockingFee.add(restockingFeeTax, mc, descriptor("Restocking fee total"));

        // === Order total — restockingFeeTotal never added here ===
