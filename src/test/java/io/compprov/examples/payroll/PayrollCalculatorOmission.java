...
/**
 * Calculation Omission attack: pretax deductions (401k + health premium) are fetched and
 * correctly summed into their own {@code pretaxDeductions} subgraph, but that subgraph is
 * silently dropped from the final net pay aggregation — the employee appears to take home pay
 * as if no pretax deductions were ever withheld.
 */
public class PayrollCalculatorOmission {
    ...
    public void calculate() {
        ...
        // === Pretax deductions (computed, but never applied below) ===
        ...
        // === Net pay — pretax deductions never subtracted here ===
        final var netPay = grossPay
                //.subtract(pretaxDeductions, mc, descriptor("After pretax deductions"))
