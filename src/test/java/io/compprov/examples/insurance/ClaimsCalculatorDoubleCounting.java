...
/**
 * Topological Accumulation Fraud (Double Counting) attack: the liability claim's deductible —
 * the same origin variable already consumed once inside that claim's own adjudication — is fed
 * into the total payout aggregation a second time via a parallel {@code add} path, inflating the
 * insurer's apparent total payout by the deductible amount under the same "Total payout" label
 * the clean calculation uses.
 */
public class ClaimsCalculatorDoubleCounting {
    ...
    public void calculate() {
        ...
        final var totalPayout = totalPayoutBeforeFee.add(liabilityDeductible, mc, descriptor("Total payout"));
