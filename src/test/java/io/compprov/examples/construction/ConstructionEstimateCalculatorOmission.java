...
/**
 * Calculation Omission attack: the profit margin — a credit entry the contractor is entitled to
 * collect — is computed correctly in its own subgraph, but it's silently dropped from the final
 * bid price, so the reported total understates what the contractor actually intends to charge.
 */
public class ConstructionEstimateCalculatorOmission {
